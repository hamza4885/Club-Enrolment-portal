...
export default function UserOptions({clubId}) {
  const [open, setOpen] = React.useState(false);
  const navigate=useNavigate();
  const user = JSON.parse(localStorage.getItem("user"));
  // Define the options for the SpeedDial actions
  const options = [
    { icon: <ListAltRounded />, name: 'PremiumMembers',func:PremiumMembers},
    { icon: <Person2Rounded />, name: 'Members',func:AllMembers},
  ...
  function PremiumMembers(){
    navigate("/clubs/PremiumMembers")
 }
 function AllMembers(){
  navigate(`/ClubMembers/${clubId}`)
}
